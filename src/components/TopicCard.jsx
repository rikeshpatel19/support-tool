...
const TopicCard = ({ name, status, theme, progress, onClick }) => {
  return (
    <button
      ...
      className="border-2 flex items-center gap-2 transition-all duration-200 h-32 rounded-xl p-4 relative shadow-hard text-left font-bold text-lg hover:-translate-y-0.5 active:scale-95"
    >
      {/* The Topic Name */}
      <span>{name}</span>
      ...
      {progress > 0 && (
        <div className="absolute bottom-5 w-32 flex-1 flex flex-col">
          {/* Percentage Text (Underneath the topic/challenge name) */}
